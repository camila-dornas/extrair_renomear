...
const studentsInfo = require ('./adulto/data/studentsInfo.json');
...
let total = 0
const colunaNome = 'Antes de tudo, nos diga quem é você?';
const colunaFoto = 'Envie uma foto SUA (Selfie ou você no seu Set up) que te represente atualmente e que não seja uma animação:'

studentsInfo.forEach(async (student) => {
  if (!(student[colunaFoto] === "")) {
    total += 1
    await createFile(
      student[colunaFoto],
      student[colunaNome]
